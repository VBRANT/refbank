...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="//reference[not(./title)]" priority="10">
		<xsl:text disable-output-escaping="yes">ERROR:Missing title</xsl:text>
	</xsl:template>
	
	<xsl:template match="//reference[not(./@type = 'url') and not(./author)]" priority="10">
		<xsl:text disable-output-escaping="yes">ERROR:Missing author</xsl:text>
	</xsl:template>
	
	<xsl:template match="//reference[starts-with(./@type, 'journal') and not(./volume) and not(./numero) and not(./issue)]" priority="10">
		<xsl:text disable-output-escaping="yes">ERROR:Missing volume number</xsl:text>
	</xsl:template>
	
	<xsl:template match="//reference[(./@type = 'journal article' or ./@type = 'book chapter') and not(./pagination) and not(./firstPage)]" priority="10">
		<xsl:text disable-output-escaping="yes">ERROR:Missing pagination</xsl:text>
	</xsl:template>
	
	<xsl:template match="//reference[starts-with(./@type, 'book') and not(./publisher) and not(./location)]" priority="10">
